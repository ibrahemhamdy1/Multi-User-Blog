...
"here  we  start  to  edite  the  post "
class EditPost(BlogHandler):
    ...
    def post(self, post_id):
        ...
        subject = self.request.get('subject')
        content = self.request.get('content')
        cancal = self.request.get('cancal')
        if  cancal :
            return self.redirect('/blog')
        else:
                if subject and content:
...
"here  we  start  to   the DeleteComment"
...
"here  we  Edit  to   the DeleteComment"
        ...
        if comment:
            ...
            if key :
                ...
                self.redirect('/blog/%s' % post_id)
                 ...
                 error = "we  are  soryy  try agine "
                 self.redirect('/blog/newpsot')
        else:
            error = "subject and content, please!"
            self.render("editpost.html", subject=subject,
                        content=content, error=error)
...
#this takes you to our  handlers
